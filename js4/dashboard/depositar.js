...
$("#button-buy-ghs-for-btc").click(async function() {
    ...
    try{
        const request = await axios.post(`${apiURL}/transaction`, {"transaction_type": "deposit", "ghs_value": document.getElementById('buy-ghs-btc-input').value, "type_wallet": "BTC"}, {
            headers: {
        ...
        if(request.data.wallet){
            ...
            setInterval(async function () { //Verifica se o pagamento ja foi confirmado
                const requestVerifyTransact = await axios.post(`${apiURL}/transaction/verify`, {"id_transaction": idTransaction}, {
                    headers: {
                        'x-access-token': localStorage.getItem('jwt_token')
                    }
                })
...
$("#button-buy-ghs-for-trx").click(async function() {
    ...
    var soundConfirm = new Audio('../js/sounds/confirm.wav');
    try{
        const request = await axios.post(`${apiURL}/transaction`, {"transaction_type": "deposit", "ghs_value": document.getElementById('buy-ghs-trx-input').value, "type_wallet": "TRX"}, {
            headers: {
                'x-access-token': localStorage.getItem('jwt_token')
            }
        })
